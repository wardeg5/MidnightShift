...
acceptReleased = keyboard_check_released(vk_space);
accepts = 0;

if(acceptKey && accepts == 0){
	accepts++;
} else if (acceptKey){
	acceptKey = false;	
}
...
tbY = camera_get_view_y(view_camera[0]) + 144;
...
if (speakerSprite[page] != noone){
	...
	draw_sprite_ext(sprite_index, image_index, _speakerX, tbY - 200,speakerSide[page] * 1,1,0,c_white,1);
}


//back of the text box
draw_sprite_ext(tbSprite[page],tbImage, _txtbX, _txtbY, tbWidth/tbSpriteW, tbHeight/tbSpriteH, 0, c_white, .66);
